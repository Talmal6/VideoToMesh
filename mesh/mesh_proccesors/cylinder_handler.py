    ...
    def _calculate_rotation_delta(self, curr_gray, prev_gray, 
            ...
            try:
                ...
                if last_mask is not None:
                    ...
                    if flow_mask.max() > 1: flow_mask[flow_mask > 0] = 255
                    
                    # כיווץ המסכה (Erosion) - קריטי!
                    # זה מבטיח שלא נעקוב אחרי הפיקסלים שבקצה הבקבוק (שמתמזגים עם הרקע)
                    # אלא רק אחרי הטקסטורה שבמרכז הגליל.
                    kernel_size = int((x2 - x1) * 0.1) # 10% מרוחב הבקבוק
                    if kernel_size > 0:
                        kernel = np.ones((kernel_size, kernel_size), np.uint8)
                        flow_mask = cv2.erode(flow_mask, kernel, iterations=1)
                else:
                    # Fallback אם אין מסכה
                ...
                p0 = cv2.goodFeaturesToTrack(prev_gray, mask=flow_mask, maxCorners=100, 
                                            qualityLevel=0.01, minDistance=5)

                if p0 is None or len(p0) < 4: return 0.0
                ...
                if len(clean_diffs) == 0: return 0.0
                ...
                # (אפשר להכפיל ב-1.5 אם מרגיש איטי מדי)
                return net_rotation_px * self.rotation_sensitivity * 1.5
